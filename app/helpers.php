...
function getPageAds(){
    ...
    $ads = App\Models\AdsBase::where(['status' => 1])->first();
    return $ads;
}

function getMetaData($slug){
    return App\Models\Seo::where(['slug' => $slug])->first();
}

function getMetaDescription($description){
    if (100 < strlen($description)) {
...
function getInArticleAd($description, $ads){
    if(isset($ads->in_article_ad) && !empty($ads->in_article_ad)){
        $in_article_ad = $ads->in_article_ad;
    }else{
        $in_article_ad = '';
    }
    $in_article_ad = '<div class="ad-section mb-0">'. $in_article_ad . '</div>';
    $description = str_replace('##in_article_ad##', $in_article_ad, $description);
    return $description;
}

function numberToOrdinal($number) {
    // Handle special case for teens
